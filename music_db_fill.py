...
tracks = [
    ...
    {'album_id': 10, 'duration': 476, 'title': 'Enjoy the Silence', 'track_id': 20}
]
...
track_mixtape = [
    ...
    {'mixtape_id': 3, 'track_id': 20},
    {'mixtape_id': 4, 'track_id': 15},
    {'mixtape_id': 5, 'track_id': 17},
    {'mixtape_id': 5, 'track_id': 5},
    {'mixtape_id': 5, 'track_id': 13},
...
def select():
    result = connection.execute("SELECT title, release_year "
                                "FROM album "
                                "WHERE release_year = 2018;").fetchall()
    pprint(result)

    result = connection.execute("SELECT title, duration "
                                "FROM track "
                                "ORDER BY duration DESC  "
                                "LIMIT 1;").fetchall()
    pprint(result)

    result = connection.execute("SELECT title "
                                "FROM track "
